...
const StyleSelector = ({value, onChange, isOpen, setIsOpen} : 
    {value: ThumbnailStyle; onChange: (style: ThumbnailStyle)=> void; isOpen: boolean; setIsOpen: (open: boolean)=> void}) => {
        ...
        const styleDesciption : Record<ThumbnailStyle, string>= {
            "Bold & Graphic" : "High contrast, bold typography, striking visuals",
            "Minimalist" : "Clean, simple, lots of white spcae",
  ...
  return (
    <div className="relative space-y-3 dark">
        ...
        className="flex w-full items-center justify-between rounded-md vorder px-4 py-3 text-left transition bg-white/8 border-white/10 text-zinc-200 hover:bg-white/12">
            <div className="space-y-1">
                <div>
                    {styleIcons[value]}
                    <span>{value}</span>
                </div>
                <p className="text-xs text-zinc-400">{styleDesciption[value]}</p>
            </div>
                ...
                {thumbnailStyles.map((style)=>(
                    <button key={style} type="button" onClick={()=>{onchange(style); setIsOpen(false);}} className="flex w-full items-start gap-3 px-4 py-3 text-left transition hover:bg-black/30">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-zinc-400" >{styleDesciption[style]}</p>
                        </div>
                     </button>
                ))}
            </div>
